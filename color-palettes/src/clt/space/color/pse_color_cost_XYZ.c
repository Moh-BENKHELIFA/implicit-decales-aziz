...
pseColorXYZCostFuncContextsMemInit
  (void* user_ctxt,
...
{
  static const struct pse_color_cost_func_implem_t IMPLEMS[PSE_COLOR_COST_FUNC_XYZ_COUNT+1] = {
  ...
  assert(implem && pseIsColorXYZCostFunctorValid(cf));

  *implem = IMPLEMS[cf-PSE_COLOR_COST_FUNC_XYZ_BEGIN-1];
  implem->ctxt_type_info.user_ctxt = (void*)((uintptr_t)components);
  implem->ctxt_type_info.meminit = pseColorXYZCostFuncContextsMemInit;
  return RES_OK;
}
   ...
   pse_relshp_cost_func_id_t* fid)
{
  ...
  if(  !cps
    || (PSE_COLOR_SPACE_FROM(fmt) == PSE_COLOR_SPACE_XYZ)
    || !pseIsColorXYZCostFunctorValid(cf)
  ...
  PSE_CALL_OR_RETURN(res, pseColorXYZCostFunctorImplemGet
